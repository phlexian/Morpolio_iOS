...
// MARK: - 2. PORTFOLIO LAYOUT (STANDART TASARIM İSKELETİ)
...
struct ContentView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            VStack(spacing: 0) {
                HStack { Spacer(); Capsule().fill(Color.gray.opacity(0.3)).frame(width: 40, height: 6).padding(.top, 10).padding(.bottom, 5); Spacer() }.background(Color(uiColor: .systemBackground))
                ...
                TabView(selection: $selectedTab) {
                    ...
                    PortfolioLayout(title: "Hisseler", themeColor: AppTheme.stockColor, totalValue: totalStockValue, selectedCurrency: $selectedCurrency, exchangeRates: $exchangeRates, isCurrencyBusy: $isCurrencyBusy, isRefreshing: isRefreshingStock, onRefresh: refreshStocks, onAdd: { showStockAdder = true }) {
                        StockListView(stocks: stocks, currency: selectedCurrency, rates: exchangeRates, themeColor: AppTheme.stockColor, onRefresh: refreshStocks, onDeleteRequest: { item in activeCategory = .stock; handleDelete(type: .stock(symbol: item.symbol, quantity: item.quantity, price: item.currentPrice), itemToDelete: item) })
                    }.sheet(isPresented: $showStockAdder) { StockAdderScreen(themeColor: AppTheme.stockColor).presentationDetents([.fraction(0.6), .large]) }.tabItem { Label("Hisse", systemImage: "chart.bar.xaxis") }.tag(0)
                    ...
                    PortfolioLayout(title: "Kripto Varlıklar", themeColor: AppTheme.cryptoColor, totalValue: totalCryptoValue, selectedCurrency: $selectedCurrency, exchangeRates: $exchangeRates, isCurrencyBusy: $isCurrencyBusy, isRefreshing: isRefreshingCrypto, onRefresh: refreshCryptos, onAdd: { showCryptoAdder = true }) {
                        CryptoListView(cryptos: cryptos, currency: selectedCurrency, rates: exchangeRates, themeColor: AppTheme.cryptoColor, onRefresh: refreshCryptos, onDeleteRequest: { item in activeCategory = .crypto; handleDelete(type: .crypto(symbol: item.symbol, quantity: item.quantity, price: item.currentPrice), itemToDelete: item) })
                    }.sheet(isPresented: $showCryptoAdder) { CryptoAdderScreen(themeColor: AppTheme.cryptoColor).presentationDetents([.fraction(0.6), .large]) }.tabItem { Label("Kripto", systemImage: "bitcoinsign.circle") }.tag(1)
                    ...
                    PortfolioLayout(title: "Yatırım Fonları", themeColor: AppTheme.fundColor, totalValue: totalFundValue, selectedCurrency: $selectedCurrency, exchangeRates: $exchangeRates, isCurrencyBusy: $isCurrencyBusy, isRefreshing: isRefreshingFund, onRefresh: refreshFunds, onAdd: { showFundAdder = true }) {
                        FundListView(funds: funds, currency: selectedCurrency, rates: exchangeRates, themeColor: AppTheme.fundColor, onRefresh: refreshFunds, onDeleteRequest: { item in activeCategory = .fund; handleDelete(type: .fund(symbol: item.symbol, quantity: item.quantity, price: item.currentPrice), itemToDelete: item) })
                    }.sheet(isPresented: $showFundAdder) { FundAdderScreen(themeColor: AppTheme.fundColor).presentationDetents([.fraction(0.6), .large]) }.tabItem { Label("Fon", systemImage: "building.columns.circle") }.tag(2)
                    ...
                    PortfolioLayout(title: "Değerli Madenler", themeColor: AppTheme.elementColor, totalValue: totalElementValue, selectedCurrency: $selectedCurrency, exchangeRates: $exchangeRates, isCurrencyBusy: $isCurrencyBusy, isRefreshing: isRefreshingElement, onRefresh: refreshElements, onAdd: { showElementAdder = true }) {
                        ElementListView(elements: elements, currency: selectedCurrency, rates: exchangeRates, themeColor: AppTheme.elementColor, onRefresh: refreshElements, onDeleteRequest: { item in activeCategory = .element; handleDelete(type: .element(symbol: item.symbol, quantity: item.quantity, price: item.currentPrice), itemToDelete: item) })
                    }.sheet(isPresented: $showElementAdder) { ElementAdderScreen(themeColor: AppTheme.elementColor).presentationDetents([.fraction(0.6), .large]) }.tabItem { Label("Maden", systemImage: "circle.hexagonpath.fill") }.tag(3)
                    ...
                    PortfolioLayout(title: "Nakit Varlıklar", themeColor: AppTheme.cashColor, totalValue: totalCashValue, selectedCurrency: $selectedCurrency, exchangeRates: $exchangeRates, isCurrencyBusy: $isCurrencyBusy, isRefreshing: isRefreshingCash, onRefresh: refreshCash, onAdd: { showCashAdder = true }) {
                        CashListView(cashItems: cashItems, currency: selectedCurrency, rates: exchangeRates, themeColor: AppTheme.cashColor, onRefresh: refreshCash, onDeleteRequest: { item in activeCategory = .cash; handleDelete(type: .cash(symbol: item.symbol, quantity: item.quantity, price: item.currentPrice), itemToDelete: item) })
                    }.sheet(isPresented: $showCashAdder) { CashAdderScreen(themeColor: AppTheme.cashColor).presentationDetents([.fraction(0.6), .large]) }.tabItem { Label("Nakit", systemImage: "banknote") }.tag(4)
                }
                .tint(currentTabColor)
            }
            if showToast { FancyToastView(message: toastMessage, themeColor: AppTheme.color(for: activeCategory), timeLeft: timeLeft, onUndo: undoDelete).padding(.bottom, 80).transition(.move(edge: .bottom).combined(with: .opacity)).zIndex(10) }
        }
        .onChange(of: selectedCurrency) { _, n in if n != .tryCurrency && exchangeRates == (1.0, 1.0) { Task { isCurrencyBusy = true; if let r = await currencyService.fetchRates() { exchangeRates = r }; isCurrencyBusy = false } } }
    }
    
    var currentTabColor: Color { switch selectedTab { case 0: return AppTheme.stockColor; case 1: return AppTheme.cryptoColor; case 2: return AppTheme.fundColor; case 3: return AppTheme.elementColor; case 4: return AppTheme.cashColor; default: return .blue } }
    ...
    // Delete/Undo Logic
    func handleDelete<T: PersistentModel>(type: DeletedItemWrapper, itemToDelete: T) { undoTimer?.invalidate(); deletedItemBackup = type; context.delete(itemToDelete); timeLeft = 5; withAnimation { showToast = true }; undoTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { _ in if timeLeft > 0 { withAnimation { timeLeft -= 1 } } else { withAnimation { showToast = false }; deletedItemBackup = nil; undoTimer?.invalidate() } } }
    func undoDelete() { undoTimer?.invalidate(); guard let b = deletedItemBackup else { return }; switch b { case .stock(let s, let q, let p): context.insert(Stock(symbol: s, quantity: q, currentPrice: p)); case .crypto(let s, let q, let p): context.insert(Crypto(symbol: s, quantity: q, currentPrice: p)); case .fund(let s, let q, let p): context.insert(Fund(symbol: s, quantity: q, currentPrice: p)); case .element(let s, let q, let p): context.insert(Element(symbol: s, quantity: q, currentPrice: p)); case .cash(let s, let q, let p): context.insert(Cash(symbol: s, quantity: q, currentPrice: p)) }; withAnimation { showToast = false }; deletedItemBackup = nil }
}
...
struct StockListView: View {
    ...
    var body: some View { List { ForEach(stocks) { stock in HStack { VStack(alignment: .leading) { Text(stock.symbol).font(.headline).bold(); Text("\(stock.quantity, specifier: "%.0f") adet").font(.caption).foregroundStyle(.gray) }; Spacer(); VStack(alignment: .trailing) { Text("\(convert(stock.totalValue), specifier: "%.2f") \(currency.rawValue)").bold(); Text("\(convert(stock.currentPrice), specifier: "%.2f") \(currency.rawValue)").font(.caption).foregroundStyle(.secondary) } }.swipeActions(edge: .trailing, allowsFullSwipe: false) { Button(role: .destructive) { onDeleteRequest(stock) } label: { Label("Sil", systemImage: "trash") }.tint(.red); Button { itemToUpdate = stock } label: { Label("Güncelle", systemImage: "pencil") }.tint(themeColor) } } }.refreshable { await onRefresh() }
...
struct CryptoListView: View {
    ...
    func convert(_ value: Double) -> Double { switch currency { case .tryCurrency: return value; case .usd: return value / (rates.usd > 0 ? rates.usd : 1); case .eur: return value / (rates.eur > 0 ? rates.eur : 1) } }
    var body: some View { List { ForEach(cryptos) { crypto in HStack { VStack(alignment: .leading) { Text(crypto.symbol).font(.headline).bold(); Text("\(crypto.quantity, specifier: "%.4f") adet").font(.caption).foregroundStyle(.gray) }; Spacer(); VStack(alignment: .trailing) { Text("\(convert(crypto.totalValue), specifier: "%.2f") \(currency.rawValue)").bold(); Text("\(convert(crypto.currentPrice), specifier: "%.2f") \(currency.rawValue)").font(.caption).foregroundStyle(.secondary) } }.swipeActions(edge: .trailing, allowsFullSwipe: false) { Button(role: .destructive) { onDeleteRequest(crypto) } label: { Label("Sil", systemImage: "trash") }.tint(.red); Button { itemToUpdate = crypto } label: { Label("Güncelle", systemImage: "pencil") }.tint(themeColor) } } }.refreshable { await onRefresh() }
...
struct FundListView: View {
    ...
    func convert(_ value: Double) -> Double { switch currency { case .tryCurrency: return value; case .usd: return value / (rates.usd > 0 ? rates.usd : 1); case .eur: return value / (rates.eur > 0 ? rates.eur : 1) } }
    var body: some View { List { ForEach(funds) { fund in HStack { VStack(alignment: .leading) { Text(fund.symbol).font(.headline).bold(); Text("\(fund.quantity, specifier: "%.0f") adet").font(.caption).foregroundStyle(.gray) }; Spacer(); VStack(alignment: .trailing) { Text("\(convert(fund.totalValue), specifier: "%.2f") \(currency.rawValue)").bold(); Text("\(convert(fund.currentPrice), specifier: "%.4f") \(currency.rawValue)").font(.caption).foregroundStyle(.secondary) } }.swipeActions(edge: .trailing, allowsFullSwipe: false) { Button(role: .destructive) { onDeleteRequest(fund) } label: { Label("Sil", systemImage: "trash") }.tint(.red); Button { itemToUpdate = fund } label: { Label("Güncelle", systemImage: "pencil") }.tint(themeColor) } } }.refreshable { await onRefresh() }
...
struct ElementListView: View {
    ...
    func convert(_ value: Double) -> Double { switch currency { case .tryCurrency: return value; case .usd: return value / (rates.usd > 0 ? rates.usd : 1); case .eur: return value / (rates.eur > 0 ? rates.eur : 1) } }
    var body: some View { List { ForEach(elements) { element in HStack { VStack(alignment: .leading) { Text(element.displayName).font(.headline).bold(); Text("\(element.quantity, specifier: "%.2f") Gr").font(.caption).foregroundStyle(.gray) }; Spacer(); VStack(alignment: .trailing) { Text("\(convert(element.totalValue), specifier: "%.2f") \(currency.rawValue)").bold(); Text("\(convert(element.currentPrice), specifier: "%.2f") \(currency.rawValue)").font(.caption).foregroundStyle(.secondary) } }.swipeActions(edge: .trailing, allowsFullSwipe: false) { Button(role: .destructive) { onDeleteRequest(element) } label: { Label("Sil", systemImage: "trash") }.tint(.red); Button { itemToUpdate = element } label: { Label("Güncelle", systemImage: "pencil") }.tint(themeColor) } } }.refreshable { await onRefresh() }
...
struct CashListView: View {
    ...
    func convert(_ value: Double) -> Double { switch currency { case .tryCurrency: return value; case .usd: return value / (rates.usd > 0 ? rates.usd : 1); case .eur: return value / (rates.eur > 0 ? rates.eur : 1) } }
    var body: some View { List { ForEach(cashItems) { item in HStack { VStack(alignment: .leading) { Text(item.displayName).font(.headline).bold(); Text("\(item.quantity, specifier: "%.2f")").font(.caption).foregroundStyle(.gray) }; Spacer(); VStack(alignment: .trailing) { Text("\(convert(item.totalValue), specifier: "%.2f") \(currency.rawValue)").bold(); if item.symbol != "TRY" { Text("Kur: \(item.currentPrice, specifier: "%.2f") ₺").font(.caption).foregroundStyle(.secondary) } } }.swipeActions(edge: .trailing, allowsFullSwipe: false) { Button(role: .destructive) { onDeleteRequest(item) } label: { Label("Sil", systemImage: "trash") }.tint(.red); Button { itemToUpdate = item } label: { Label("Güncelle", systemImage: "pencil") }.tint(themeColor) } } }.refreshable { await onRefresh() }
